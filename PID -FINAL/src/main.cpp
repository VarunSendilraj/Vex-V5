...
void tiltPID(double target)
 {    //target is what tiltMotor.rotation(rotationUnits::deg) returns when forward
double kp = 0.0;     
double ki = 0.0;
double kd = 0.0;
...
vex::task::sleep(50);
while(std::abs(error) > 3){    // will go until tray is within 3 degrees of target;
    error = target - Tilter.rotation(rotationUnits::deg);
    if(error == 0){
        break;
    }
    proportion = kp * error;
    ...
    Tilter.spin(fwd, finalPower, velocityUnits::pct);
    vex::task::sleep(20);
}
Tilter.stop();
 }
 void buttonTilt (void) {
   tiltPID(100); // 100 refers to the target area
 }
...
/////
  ...
  /////////////////////////////////////////////////
  vex::task::sleep(1000);
  tiltPID(100); // 100 refers to the target 
  ///////////////////////////////////////////////
  vex::task::sleep(1000);
  resetDriveSensors = true;
  desiredValue = -100; // move backwards 100
  desiredTurnValue = 0; 
}
...
//tilterControl
  ...
  //Tilter Control 2 
   ...
   if(Controller1.ButtonRight.pressing()) 
     { Tilter.spin(directionType::fwd, TiltSpeedPCT, velocityUnits::pct);
     RightIntakeMotor.setStopping(coast);
     LeftIntakeMotor.setStopping(coast);
    }

    else if(Controller1.ButtonLeft.pressing()) {
      Tilter.spin(directionType::rev, TiltSpeedPCT, velocityUnits::pct);
      RightIntakeMotor.setStopping(coast);
      LeftIntakeMotor.setStopping(coast);
    }
    else{
